...
// In production, NODE_ENV will be "production" and DEMO_MODE should be "0"
export const isDevMode = () => {
  return process.env.NODE_ENV !== "production" || process.env.DEMO_MODE === "1";
};

// Demo users (not used in production)
...
export const authOptions: NextAuthOptions = {
  // Use Prisma adapter in production
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          // In production mode, isDevMode returns false so demo logic is skipped
  ...
  callbacks: {
    ...
    async signIn({ user, account, profile }) {
      // In production, this callback can be used for custom logic if needed
      return true;
    },
  },
...
export async function registerUser(name: string, email: string, password: string) {
  if (isDevMode()) {
    console.log("Demo mode: Simulating user registration for", email);
